...
def test_forward():
    ...
    expected_batch = np.array([[[[6, 8],
                          [14, 16]]]])

    # Verify output matches expected pooled values
    assert np.array_equal(output_batch, expected_batch), \
...
def test_backward():
    ...
    dL_dout_batch = np.array([[[[1, 2],
                         [3, 4]]]])
    dL_dinput_batch = pool.backward(dL_dout_batch)

    expected_batch = np.array([[[[0, 0, 0, 0],
                               [0, 1, 0, 2],
                               [0, 0, 0, 0],
                               [0, 3, 0, 4]]]])

    assert np.array_equal(dL_dinput_batch, expected_batch), \
